 ...
 *
 * The list is read from the engine's own resources, once, on first use. **Nothing here touches
...
object PublicSuffix {
    ...
    @Volatile private var cached: Parsed? = null

    private fun load(): Parsed = cached ?: synchronized(this) {
        cached ?: run {
            val stream = PublicSuffix::class.java.getResourceAsStream(RESOURCE)
                ?: error("bundled $RESOURCE is missing from the engine's resources")
    ...
    internal fun toAsciiLabel(label: String): String? {
        ...
        return runCatching { IDN.toASCII(lower, IDN.ALLOW_UNASSIGNED).lowercase() }
            .getOrNull()
