...
class DateFactsTest {
    ...
    fun `finds an iso date`() {
        ...
        assertEquals(LocalDate(2026, 9, 22), found.single().date)
    ...
    fun `flags a genuinely ambiguous numeric date and reports both readings`() {
        ...
        assertEquals(LocalDate(2026, 4, 3), match.date)
        assertEquals(LocalDate(2026, 3, 4), match.alternate)
    }
    ...
    fun `a numeric date with a day above twelve is unambiguous`() {
        ...
        assertEquals(LocalDate(2026, 12, 25), match.date)
    ...
    fun `identical readings are not ambiguous`() {
        ...
        assertFalse(match.ambiguous)
        assertEquals(LocalDate(2026, 5, 5), match.date)
    }

    @Test
    fun `month-first preference flips the reading`() {
        val match = DateFacts.find("03/04/2026", dayFirst = false).single()
        assertEquals(LocalDate(2026, 3, 4), match.date)
        assertEquals(LocalDate(2026, 4, 3), match.alternate)
    ...
    fun `finds several dates in order of appearance`() {
        ...
        assertEquals(
            listOf(LocalDate(2020, 1, 15), LocalDate(2030, 1, 14)),
            found.map { it.date },
        )
    }
    ...
    fun `counts days until a date - negative once passed`() {
        val from = LocalDate(2026, 9, 22)
        assertEquals(10, DateFacts.daysUntil(LocalDate(2026, 10, 2), from))
        assertEquals(-1, DateFacts.daysUntil(LocalDate(2026, 9, 21), from))
    ...
    fun `applies a validity rule like Schengen's six months`() {
        ...
        val passportExpiry = LocalDate(2027, 1, 22) // four months away
        assertTrue(
    ...
    fun `an already-expired date counts as within any window`() {
        val today = LocalDate(2026, 9, 22)
        assertTrue(DateFacts.expiresWithin(LocalDate(2020, 1, 1), today, months = 6))
    }

    @Test
    fun `rejects a negative window`() {
        assertFailsWith<IllegalArgumentException> {
            DateFacts.expiresWithin(LocalDate(2026, 1, 1), LocalDate(2026, 1, 1), -1)
        }
    }
}
