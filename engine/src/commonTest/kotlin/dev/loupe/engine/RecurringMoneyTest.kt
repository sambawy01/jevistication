...
class RecurringMoneyTest {
    ...
    private fun monthly(merchant: String, months: Int, amount: Long, from: LocalDate) =
        (0 until months).map { Charge(merchant, from.plus(it.toLong(), DateTimeUnit.MONTH), amount) }

    @Test
    fun `detects a monthly subscription`() {
        val charges = monthly("Streamly", 6, 999, LocalDate(2026, 4, 22))
    ...
    fun `recognises weekly - quarterly and annual schedules`() {
        val weekly = (0 until 8).map { Charge("W", LocalDate(2026, 7, 1).plus(it.toLong(), DateTimeUnit.WEEK), 500) }
        val quarterly = (0 until 4).map { Charge("Q", LocalDate(2025, 1, 1).plus(it * 3L, DateTimeUnit.MONTH), 4500) }
        val annual = (0 until 3).map { Charge("A", LocalDate(2023, 5, 1).plus(it.toLong(), DateTimeUnit.YEAR), 9900) }
    ...
    fun `calls a scattered series irregular rather than inventing a schedule`() {
        val charges = listOf(
            Charge("Corner Shop", LocalDate(2026, 1, 3), 400),
            Charge("Corner Shop", LocalDate(2026, 2, 19), 1200),
            Charge("Corner Shop", LocalDate(2026, 8, 2), 250),
        )
    ...
    fun `two charges are a gap - not a pattern`() {
        val charges = monthly("Twice", 2, 500, LocalDate(2026, 8, 22))
    ...
    fun `uses the median so one odd charge does not skew the typical amount`() {
        val charges = listOf(
            Charge("M", LocalDate(2026, 5, 1), 1000),
            Charge("M", LocalDate(2026, 6, 1), 1000),
            Charge("M", LocalDate(2026, 7, 1), 50000), // one annual top-up
            Charge("M", LocalDate(2026, 8, 1), 1000),
        )
    ...
    fun `finds the ones still charging but long dormant`() {
        val active = monthly("Active", 4, 999, LocalDate(2026, 6, 22))
        val stale = monthly("Forgotten", 4, 1499, LocalDate(2025, 1, 22))
    ...
    fun `ranks by total spend`() {
        val cheap = monthly("Cheap", 5, 100, LocalDate(2026, 4, 22))
        val pricey = monthly("Pricey", 5, 5000, LocalDate(2026, 4, 22))
        assertEquals(
            listOf("Pricey", "Cheap"),
