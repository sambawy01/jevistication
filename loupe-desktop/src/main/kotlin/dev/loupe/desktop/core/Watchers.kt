...
import dev.loupe.sources.ItemKind
...
/**
 * Runs the five watchers over scanned items (C3). Each uses the engine's own watcher, unchanged;
 * this layer only turns scanned items into the inputs those watchers take.
 *
 * What the desktop can and cannot do, stated rather than implied:
 *
 * - **Expiry radar** — the date arithmetic is mechanical and always runs; deciding *what a
 *   document is* needs the model, so the full radar runs only when it is loaded.
 ...
 * - **Impersonation** — there is no address book on the desktop, so a contact is inferred from
 *   history: a display name used at least twice from the same address.
    ...
    fun termChanges(items: List<SourceItem>): List<TermChangeFinding> {
        val groups = items.groupBy { item ->
            if (item.kind == ItemKind.EMAIL) {
                ...
                "file:" + item.path.parent + "/" + item.path.fileName.toString().replace(Regex("""\d+"""), "#")
            }
        }
