...
- **2026-09-25 — iPhone: the mascot renders right again, and Laya is required up front (owner's report
  ...
  Later → Sorting locked, Get Laya from the lock; with the model → no step, Run now offered). **Model host
  set (owner decision, same day):** Supabase Storage, public bucket `models`, `source.host`
  `https://zqyeihzjpfnvkrprcwam.supabase.co`, `urlTemplate` `{host}/storage/v1/object/public/models/laya/v1/{file}`
  (`ModelDeliveryTests` pins the three resolved URLs). v1 is never overwritten; a re-export goes to `laya/v2/` with
  new pins. Resume: the bucket serves `Accept-Ranges: bytes`, a strong `ETag` and `Last-Modified`, and answers a
  `Range` + matching `If-Range` (either validator) with 206 and a wrong one with 200 — what URLSession resume data
...
| 6 | Web tab: Flights — key onboarding, search, results ranked on device, "Online" labels, off switch | **Done on the simulator 2026-09-23, Laya wired in** — ranked by Laya with the rule baseline alongside; model download from the Supabase host since 2026-09-25; Duffel end to end needs a test key |
...
**Epic #7 is complete on the simulator** except these owner-blocked items: an iPhone (device fps/latency, battery/thermal for passive mode, Diagnostics run), an Apple developer account (Team ID, App Group for the Share Extension on a device), and Google/Microsoft OAuth client IDs for mail/calendar sign-in.
