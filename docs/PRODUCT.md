...
Evidence behind the design decisions is in [`RESEARCH.md`](RESEARCH.md). Licence provenance for
every model considered is in [`LICENSING.md`](LICENSING.md).
...
**Set selection.** Choice takes many candidates at once, so *"pick the best of these 200"* is
one call: best shot in a burst, which duplicate to keep, which of 40 notes answers this, which
of 200 flight options fits your priorities.
...
**Stated limits, which we publish rather than hide.** Hard-tier accuracy on models of this class
is around 37%; a well-crafted novel phishing page is a hard case. A small local model is not
adversarially robust. This is supplementary to browser protections, not a replacement. The
...
Published tier numbers for models of this class: **easy 87.5%, standard 69.4%, hard 36.9%** at
11.8% ECE. The telling detail is that inference fixes which lifted the standard tier by +6.9
points moved the hard tier by **exactly zero**. Hard means multi-hop reasoning or genuinely
contested judgment — an analogous breakdown shows logical deduction at 100% and causal
attribution at 55%, and humans disagree on causal attribution too.
...
on a discretionary policy review. Note also that full UI control hands a model that is around
...
1. **Which model ships as the default.** No backend dominates: per-task accuracy ranges widely
   and the leader flips by task.
...
1. Licence verification at source; decision model on-device behind the backend interface;
   mechanical extractors; text-state pipeline
