...
enum MS {
    ...
    static let table: [String: (String, String)] = [
        ...
        "intro": ("How the Loupe Decision Model on this iPhone is used: which model reads, how much text, and when an answer is accepted. Changes are saved at once and apply to the next run — no restart.",
                  "كيف يُستخدم نموذج Loupe للقرار على هذا الـ iPhone: أي نموذج يقرأ، وكم من النص، ومتى يُعتمد الجواب. تُحفظ التغييرات فورًا وتسري على التشغيل التالي، دون إعادة تشغيل."),
        ...
        "savedMemory": ("Saved. The decision model is loading or unloading in the background.", "حُفظ. يجري تحميل نموذج القرار أو تفريغه في الخلفية."),
        "cancel": ("Cancel", "إلغاء"),
        "done": ("Done", "تم"),
        "loadedNow": ("The decision model is loaded in memory now.", "نموذج القرار محمّل في الذاكرة الآن."),
        "unloadedNow": ("The decision model is not in memory now; the next run loads it.", "نموذج القرار غير محمّل في الذاكرة الآن؛ يحمّله التشغيل التالي."),
        ...
        "route.phone": ("iPhone carries one model, the multilingual decision model, so Automatic and Multilingual both run it. The English model is on Loupe Station only; choosing it here still runs multilingual.",
                        "يحمل الـ iPhone نموذجًا واحدًا هو نموذج القرار متعدد اللغات، لذا يشغّله الوضعان التلقائي ومتعدد اللغات. النموذج الإنجليزي في Loupe Station فقط؛ واختياره هنا يشغّل متعدد اللغات أيضًا."),
        "mem.full": ("Full", "كامل"),
        ...
        "g.memory_mode.desc": ("How long the decision model stays in the phone's memory: faster answers against memory and battery.",
                               "كم يبقى نموذج القرار في ذاكرة الهاتف: أجوبة أسرع مقابل الذاكرة والبطارية."),
        "mem.desc.full": ("Keeps the decision model loaded once opened: the fastest answers, but it holds several hundred MB of memory and iOS is more likely to close Loupe in the background.",
                          "يُبقي نموذج القرار محمّلًا بعد فتحه: أسرع الأجوبة، لكنه يحجز مئات الميغابايتات من الذاكرة ويزيد احتمال أن يغلق iOS تطبيق Loupe في الخلفية."),
        "mem.desc.balanced": ("Frees the decision model after it sits idle for the minutes below; the next use waits a few seconds while it loads and its files are checked again.",
                              "يحرّر نموذج القرار بعد أن يبقى خاملًا للدقائق أدناه؛ ينتظر الاستخدام التالي بضع ثوانٍ حتى يُحمَّل وتُفحص ملفاته مجددًا."),
        "mem.desc.low": ("Loads the decision model for each run and frees it right after: the least memory and battery held between runs; every run starts a few seconds slower.",
                         "يحمّل نموذج القرار لكل تشغيل ويحرّره فور انتهائه: أقل ذاكرة وبطارية محجوزة بين التشغيلات؛ ويبدأ كل تشغيل أبطأ ببضع ثوانٍ."),
        "applies.memory_mode": ("Applies now: the decision model loads or unloads in the background.", "يسري الآن: يُحمَّل نموذج القرار أو يُفرَّغ في الخلفية."),
        ...
        "g.idle_unload_min.desc": ("Balanced only. Shorter frees memory sooner; the next use then waits a few seconds for the decision model to load. 0 = never.",
                                   "للمتوازن فقط. المدة الأقصر تحرر الذاكرة أسرع، لكن الاستخدام التالي ينتظر بضع ثوانٍ حتى يُحمَّل نموذج القرار. 0 = أبدًا."),
        ...
        "g.use_calibration.desc": ("Once your corrections have fitted a calibration for a question, the decision model's confidence is adjusted with it. Off: raw confidence. No calibration is fitted on iPhone yet, so on and off both use raw confidence today.",
                                   "حين تكفي تصحيحاتك لمعايرة سؤال، تُعدَّل ثقة نموذج القرار بها. عند الإيقاف: الثقة الخام. لا تُجرى معايرة على الـ iPhone بعد، لذا يستخدم الوضعان الثقة الخام اليوم."),
        "g.rules_first": ("Rules first", "القواعد أولًا"),
        "g.rules_first.desc": ("A rule that is precise where it fires (an exact duplicate of a file already judged) answers before the decision model. Off: the decision model answers those too — slower, more battery.",
                               "القاعدة الدقيقة حيث تنطبق (نسخة مطابقة لملف حُكم عليه) تجيب قبل نموذج القرار. عند الإيقاف: يجيب نموذج القرار عنها أيضًا — أبطأ، وبطارية أكثر."),
        "g.baseline_switch": ("Switch to keyword rules when they win", "التحول إلى قواعد الكلمات حين تتفوق"),
        "g.baseline_switch.desc": ("Where your corrections show a judgment's keyword rule beating the decision model, the rule answers it. Off: the decision model always answers. A judgment's own Auto / Always baseline / Always model choice still wins.",
                                   "حيث تُظهر تصحيحاتك أن قاعدة الكلمات لحكمٍ تتفوق على نموذج القرار، تجيب القاعدة عنه. عند الإيقاف: يجيب نموذج القرار دائمًا. اختيار الحكم نفسه (تلقائي / القاعدة دائمًا / نموذج القرار دائمًا) يبقى الغالب."),
        "g.bias_correction": ("Bias correction", "تصحيح الانحياز"),
        "g.bias_correction.desc": ("Loupe Station can re-weight the decision model's answers against its lean towards some options. Measured there, both methods answered worse, so the default is Off. iPhone implements Off only; the other choices are kept for Loupe Station and run as Off here.",
                                   "يمكن لـ Loupe Station إعادة وزن أجوبة نموذج القرار لتعويض ميله إلى بعض الخيارات. وقد قيس ذلك هناك فكانت الطريقتان أسوأ، لذا الافتراضي متوقف. الـ iPhone ينفّذ «متوقف» فقط؛ والخياران الآخران محفوظان لـ Loupe Station ويعملان هنا كـ «متوقف»."),
        "bias.off": ("Off", "متوقف"),
        ...
        "g.text_chars_multilingual.desc": ("Characters sent to the decision model for a feature set to \"Same as everywhere\". More context against speed and battery.",
                                           "عدد الأحرف المرسلة إلى نموذج القرار للميزة المضبوطة على «كما في كل مكان». سياق أكثر مقابل السرعة والبطارية."),
        ...
        "g.cost_input_per_mtok.desc": ("Dollars per million input tokens of the cloud model the live views compare the decision model with. Default: Claude Sonnet 5 list price. Nothing is ever sent to it.",
                                       "دولارات لكل مليون رمز إدخال للنموذج السحابي الذي تقارن به العروض المباشرة نموذج القرار. الافتراضي: سعر Claude Sonnet 5 المعلن. لا يُرسل إليه شيء أبدًا."),
        ...
        "feat.game.hint": ("The decision model flying the plane in Watch mode.", "نموذج القرار يقود الطائرة في وضع المشاهدة."),
        ...
        "fk.use_laya": ("Use the decision model", "استخدام نموذج القرار"),
        ...
        "fk.time_limit_s": ("The decision model's time limit per page", "مهلة نموذج القرار لكل صفحة"),
        "fk.queue_size": ("Pages that may wait", "الصفحات التي يمكنها الانتظار"),
        "fk.max_decisions_per_s": ("Model decisions per second, at most", "قرارات نموذج القرار في الثانية، بحدٍّ أقصى"),

        "f.judgments.use_laya": ("On: the decision model answers each judgment. Off: rules only — exact duplicates and each judgment's keyword rule answer, faster and with less battery; items no rule covers wait for a run with the decision model.",
                                 "مُفعّل: يجيب نموذج القرار عن كل حكم. متوقف: القواعد فقط — تجيب النسخ المطابقة وقاعدة الكلمات لكل حكم، أسرع وبطارية أقل؛ وتنتظر العناصر التي لا تغطيها قاعدة تشغيلًا مع نموذج القرار."),
        "f.scan.use_laya": ("On iPhone this check runs on Loupe Station's rules; the decision model does not label files here yet. Off marks each run as rules only.",
                            "على الـ iPhone يعمل هذا الفحص بقواعد Loupe Station؛ لا يصنّف نموذج القرار الملفات هنا بعد. الإيقاف يَسِم كل تشغيل بأنه من القواعد فقط."),
        ...
        "f.email.use_laya": ("On iPhone mail triage runs on the phishing evidence and Station's rules; the decision model does not read mail here yet. Off marks each run as rules only.",
                             "على الـ iPhone يعمل فرز البريد بأدلة التصيّد وقواعد Loupe Station؛ لا يقرأ نموذج القرار البريد هنا بعد. الإيقاف يَسِم كل تشغيل بأنه من القواعد فقط."),
        "f.browser.use_laya": ("On iPhone site checks use the address and the online checks only; the decision model does not read pages here. Off marks each run as rules only.",
                               "على الـ iPhone تعتمد فحوص المواقع على العنوان والفحوص عبر الإنترنت فقط؛ لا يقرأ نموذج القرار الصفحات هنا. الإيقاف يَسِم كل تشغيل بأنه من القواعد فقط."),
        "f.browser.time_limit_s": ("How long a page waits for the decision model before it gets the address check alone.", "كم تنتظر الصفحة نموذج القرار قبل أن تحصل على فحص العنوان وحده."),
        "f.browser.queue_size": ("How many pages may wait for the decision model while it reads another.", "كم صفحة يمكنها انتظار نموذج القرار وهو يقرأ غيرها."),
        "f.watchers.use_laya": ("On: the expiry watcher asks the decision model what each document is. Off: the watchers run their rules only and say the decision model check did not run.",
                                "مُفعّل: يسأل مراقب انتهاء الصلاحية نموذج القرار عن نوع كل مستند. متوقف: يعمل المراقِبون بقواعدهم فقط ويذكرون أن فحص نموذج القرار لم يجرِ."),
        "f.flights.use_laya": ("On: the decision model ranks offers against your priorities. Off: the rules rank them (price, then convenience); faster, no model needed.",
                               "مُفعّل: يرتّب نموذج القرار العروض حسب أولوياتك. متوقف: ترتّبها القواعد (السعر ثم الراحة)؛ أسرع ولا حاجة إلى النموذج."),
        ...
        "f.game.max_decisions_per_s": ("Caps how often the decision model is asked while it flies. Lower saves battery and heat; the plane reacts less often (the safety override still steers). Off = as often as the game asks.",
                                       "يحدّ عدد مرات سؤال نموذج القرار أثناء القيادة. الأقل يوفّر البطارية والحرارة؛ وتستجيب الطائرة أقل (ويبقى تجاوز السلامة يوجّهها). متوقف = بقدر ما تطلب اللعبة."),
        "f.routing": ("Which model reads for this feature.", "أي نموذج يقرأ لهذه الميزة."),
        "f.text_chars": ("How much text the decision model reads per item. More context can help; each decision is slower and uses more battery.",
                         "كم من النص يقرأ نموذج القرار لكل عنصر. السياق الأكثر قد يساعد؛ وكل قرار أبطأ ويستهلك بطارية أكثر."),
        "f.text_chars.none": ("The decision model reads no text for this feature on iPhone yet, so the limit changes nothing here.",
                              "لا يقرأ نموذج القرار نصًا لهذه الميزة على الـ iPhone بعد، لذا لا يغيّر الحد شيئًا هنا."),

        // Banners
        "banner.layaOff": ("The decision model was off for this run — answers come from rules only.", "كان نموذج القرار متوقفًا في هذا التشغيل — الأجوبة من القواعد فقط."),
        "banner.turnOn": ("Turn it on", "شغّله"),
        "banner.game": ("The decision model is off for the game (Model settings), so the baseline autopilot is flying.", "نموذج القرار متوقف للعبة (إعدادات النموذج)، لذا يقود الطيار الآلي الأساسي."),
    ]
}
