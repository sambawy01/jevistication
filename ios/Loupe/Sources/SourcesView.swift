...
/// labelled as sample data at every mention; phone sources are listed but not yet available.
struct SourcesView: View {
    @ObservedObject var sources: SourcesService

    private static let phoneSources: [(String, String)] = [
        ("Photos", "photo.on.rectangle"), ("Files", "folder"), ("Mail", "envelope"),
        ("Calendar", "calendar"), ("Contacts", "person.crop.circle"),
    ]
        ...
        NavigationStack {
            List {
                ...
                Section("Phone sources") {
                    ForEach(Self.phoneSources, id: \.0) { name, symbol in
                        HStack {
                            Label(name, systemImage: symbol)
                            Spacer()
                            Text("Coming in child 7").font(.footnote).foregroundStyle(Palette.inkSoft)
                        }
                        .foregroundStyle(Palette.inkSoft)
                        .accessibilityElement(children: .combine)
                        .accessibilityIdentifier("sources.phone.\(name.lowercased())")
                    }
                    .disabled(true)
                }
